...
ClassImp(AliAnalysisTaskKzeroshort)

AliAnalysisTaskKzeroshort::AliAnalysisTaskKzeroshort() 
  : AliAnalysisTaskSE(),
  //Output lists
  fOutput(0), 
  ...
  fHistInvMassK0ShortBefSel(0),
  fHistInvMassK0ShortAfterSel(0),
...
AliAnalysisTaskKzeroshort::AliAnalysisTaskKzeroshort(const char *name) 
...
void AliAnalysisTaskKzeroshort::UserCreateOutputObjects()
{
  ...
  fHistEvent->GetXaxis()->SetBinLabel(2, "kINT7");  
  fHistEvent->GetXaxis()->SetBinLabel(3, "Double K0S");  
  fHistEvent->GetXaxis()->SetBinLabel(4, "Triple K0S");  
  ...
  fHistTopV0Radius          = new TH1D( "fHistTopV0Radius",";Decay Radius (cm);Counts",200,-100.,100);
  ...
  fHistInvMassK0ShortBefSel  = new TH1D( "fHistInvMassK0ShortBefSel"   , ";M(#pi^{+},#pi^{-})"   ,500,0.25,0.75);
  fHistInvMassK0ShortAfterSel  = new TH1D( "fHistInvMassK0ShortAfterSel"   , ";M(#pi^{+},#pi^{-})"   ,500,0.25,0.75);

  fOutput->Add(  fHistInvMassK0ShortBefSel    );
  fOutput->Add(   fHistInvMassK0ShortAfterSel    );
...
//________________________________________________________________________
void AliAnalysisTaskKzeroshort::UserExec(Option_t *) 
{
...
//------------------------------------------------
  ...
  Float_t PL = lDistOverTotMom*0.497611;
  fHistTopV0Radius        -> Fill( lDistOverTotMom ) ; 
  ...
  if( lDcaNegToPrimVertex > fV0Sels[1] && lDcaPosToPrimVertex > fV0Sels[2]      && 
   ...
   fHistInvMassK0ShortBefSel  -> Fill (  lInvMassK0s        )   ;
   ...
   if (TMath::Abs(lNegEta) > 0.8) continue;
   if (TMath::Abs(lPosEta) > 0.8) continue;
   if (lDcaPosToPrimVertex < 0.1) continue;
   if (lDcaNegToPrimVertex < 0.1) continue;
   if (0.497611*v0->DecayLengthV0(lBestPrimaryVtxPos)/(TMath::Sqrt( pow( v0->Px(),2)+ pow( v0->Py(),2) + pow(v0->Pz(),2))) > 30.) continue; 
   if (TMath::Abs(lNSigmasPosPion) > 6) continue;
   if (TMath::Abs(lNSigmasNegPion) > 6) continue;
   if (lDcaV0Daughters > 1.0) continue;
   if (lV0CosineOfPointingAngle < 0.96) continue; 
   if (lV0Radius < 1.) continue; 
   if (TMath::Abs(lInvMassK0s - 0.497611 ) > 0.03) continue;

   fHistInvMassK0ShortAfterSel  -> Fill (  lInvMassK0s        )   ;
  
   k0scounter++;

   }// This is the end of the V0 loop

   if (k0scounter>1) fHistEvent->Fill(2.5);
   if (k0scounter>2) fHistEvent->Fill(3.5);
...
//________________________________________________________________________
void AliAnalysisTaskKzeroshort::Terminate(Option_t *)
{
