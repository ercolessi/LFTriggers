AliAnalysisTaskStrAODCfrO2 *AddTaskStrAODCfrO2(bool isMC=kTRUE, bool IsOOBPileUpRem=kTRUE, TString suffix="", bool IsV0Offline=0)
{

    // analysis manager
    AliAnalysisManager *mgr = AliAnalysisManager::GetAnalysisManager();
    if (!mgr) { ::Error("AddTaskStrAODCfrO2", "No analysis manager to connect to."); return NULL; }
    if (!mgr->GetInputEventHandler()) { ::Error("AddTaskStrAODCfrO2", "This task requires an input event handler"); return NULL; }

    // Create the task and add it to the manager
    TString combinedName=Form("StrAODCfrO2_Task_%s", suffix.Data());
    AliAnalysisTaskStrAODCfrO2 *mytask = new AliAnalysisTaskStrAODCfrO2(combinedName);
    mytask->SetMC(isMC);
    mytask->SetOOBPU(IsOOBPileUpRem);
    mytask->SetV0Offline(IsV0Offline);
    mgr->AddTask(mytask);

    // output file name
    TString outputFileName = AliAnalysisManager::GetCommonFileName();
    outputFileName += ":PWGLF_StrAODCfrO2";
    if (IsOOBPileUpRem)    outputFileName += "_OOBPileUpRemoved";
    printf("Set OutputFileName : \n %s\n", outputFileName.Data() );

    //output containers

    AliAnalysisDataContainer *coutput_0, *coutput_1, *coutput_2, *coutput_3, *coutput_4, *coutput_5, *coutput_6, *coutput_7;
    coutput_0 = mgr->CreateContainer(Form("chists_eve_%s",combinedName.Data()), TList::Class(), AliAnalysisManager::kOutputContainer, outputFileName );
    coutput_1 = mgr->CreateContainer(Form("chists_V0_%s",combinedName.Data()), TList::Class(), AliAnalysisManager::kOutputContainer, outputFileName );
    coutput_2 = mgr->CreateContainer(Form("chists_Casc_%s",combinedName.Data()), TList::Class(), AliAnalysisManager::kOutputContainer, outputFileName );
    coutput_3 = mgr->CreateContainer(Form("AliEventCuts_%s",combinedName.Data()), TList::Class(), AliAnalysisManager::kOutputContainer, outputFileName);

    //connecting input and output
    mgr->ConnectInput (mytask, 0, mgr->GetCommonInputContainer());
    mgr->ConnectOutput(mytask, 1, coutput_0);
    mgr->ConnectOutput(mytask, 2, coutput_1);
    mgr->ConnectOutput(mytask, 3, coutput_2);
    mgr->ConnectOutput(mytask, 4, coutput_3);
    return mytask;

}
