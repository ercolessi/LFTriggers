...
ClassImp(AliAnalysisTaskQAMultistrangev2)



//------------------------------------------------------------
AliAnalysisTaskQAMultistrangev2::AliAnalysisTaskQAMultistrangev2() 
  : AliAnalysisTaskSE           (), 
  fisMC{false},
  fAnalysisType{"ESD"},
  ...
  fEtaCutOnDaughterTracks{0.8},
            ...
            hDCAPosToPV{nullptr},
            hDCANegToPV{nullptr},
            hDCABachToPV{nullptr},
            hMassXiMinus{nullptr},
            hMassXiPlus{nullptr},
            hXiPt{nullptr},
            hMassOmegaMinus{nullptr},
            hMassOmegaPlus{nullptr},
            hLambdaMass{nullptr},
...
//----------------------------------------------------------------------------
AliAnalysisTaskQAMultistrangev2::AliAnalysisTaskQAMultistrangev2(const char *name) 
  ...
  fHistGenVarPhiOmegaPlus{nullptr}
{
  ...
  AliLog::SetClassDebugLevel("AliAnalysisTaskQAMultistrangev2",1);
}
...
//-----------------------------------------------------------
void AliAnalysisTaskQAMultistrangev2::UserCreateOutputObjects() 

{
  ...
  if (!fHistGenVarPhiOmegaPlus) {
   ...
   if (!hCascCosPA) {
      hCascCosPA = new TH1F("hCascCosPA","",1000,-1.,1.);
     fListHistMultistrangeQA->Add(hCascCosPA);
   }
   if (!hCascRadius) {
      hCascRadius = new TH1F("hCascRadius","",1000,0.,100.);
     fListHistMultistrangeQA->Add(hCascRadius);
   }
   if (!hDCACascDau) {
      hDCACascDau = new TH1F("hDCACascDau","",1000,0.,10.);
     fListHistMultistrangeQA->Add(hDCACascDau);
   }
   if (!hDCAV0Dau) {
      hDCAV0Dau = new TH1F("hDCAV0Dau","",1000,0.,10.);
     fListHistMultistrangeQA->Add(hDCAV0Dau);
   }
   if (!hV0CosPA) {
      hV0CosPA = new TH1F("hV0CosPA","",1000,-1.,1.);
     fListHistMultistrangeQA->Add(hV0CosPA);
   }
   if (!hV0Radius) {
      hV0Radius = new TH1F("hV0Radius","",1000,0.,100.);
     fListHistMultistrangeQA->Add(hV0Radius);
   }
   if (!hDCAV0ToPV) {
      hDCAV0ToPV = new TH1F("hDCAV0ToPV","",1000,0.,10.);
     fListHistMultistrangeQA->Add(hDCAV0ToPV);
   }
   if (!hDCAPosToPV) {
      hDCAPosToPV = new TH1F("hDCAPosToPV","",1000,0.,10.);
     fListHistMultistrangeQA->Add(hDCAPosToPV);
   }
   if (!hDCANegToPV) {
      hDCANegToPV = new TH1F("hDCANegToPV","",1000,0.,10.);
     fListHistMultistrangeQA->Add(hDCANegToPV);
   }
   if (!hDCABachToPV) {
      hDCABachToPV = new TH1F("hDCABachToPV","",1000,0.,10.);
     fListHistMultistrangeQA->Add(hDCABachToPV);
   }
   if (!hMassXiMinus) {
      hMassXiMinus = new TH1F("hMassXiMinus","",1000,0.,10.);
     fListHistMultistrangeQA->Add(hMassXiMinus);
   }
   if (!hMassXiPlus) {
      hMassXiPlus = new TH1F("hMassXiPlus","",1000,0.,10.);
     fListHistMultistrangeQA->Add(hMassXiPlus);
   }
   if (!hXiPt) {
      hXiPt = new TH1F("hXiPt","",1000,0.,10.);
     fListHistMultistrangeQA->Add(hXiPt);
   }
   if (!hMassOmegaMinus) {
      hMassOmegaMinus = new TH1F("hMassOmegaMinus","",1000,0.,10.);
     fListHistMultistrangeQA->Add(hMassOmegaMinus);
   }
   if (!hMassOmegaPlus) {
      hMassOmegaPlus = new TH1F("hMassOmegaPlus","",1000,0.,10.);
     fListHistMultistrangeQA->Add(hMassOmegaPlus);
   }
   if (!hLambdaMass) {
      hLambdaMass = new TH1F("hLambdaMass","",1000,0.,10.);
     fListHistMultistrangeQA->Add(hLambdaMass);
   }
...
//------------------------------------------------------
void AliAnalysisTaskQAMultistrangev2::UserExec(Option_t *) 

{
 ...
 /*   if (TMath::Abs(etaBach) > 0.8) { AliWarning("ERROR: bachelor eta > maxlimit");          continue; }
    ...
    // Fill the Histograms
            ...
            hCascCosPA->Fill(lXiCosineOfPointingAngle);
            hCascRadius->Fill(lXiRadius);
            hDCACascDau->Fill(lDcaXiDaughters);
            hDCAV0Dau->Fill(lDcaV0DaughtersXi);
            hV0CosPA->Fill(lV0toXiCosineOfPointingAngle);
            hV0Radius->Fill(lV0RadiusXi);
            hDCAV0ToPV->Fill(lDcaV0ToPrimVertexXi);
            hDCAPosToPV->Fill(lDcaPosToPrimVertexXi);
            hDCANegToPV->Fill(lDcaNegToPrimVertexXi);
            hDCABachToPV->Fill(lDcaBachToPrimVertexXi);
            hMassXiMinus->Fill( lInvMassXiMinus );
            hMassXiPlus->Fill( lInvMassXiPlus );
            hXiPt->Fill(lXiTransvMom);
            hMassOmegaMinus->Fill( lInvMassOmegaMinus );
            hMassOmegaPlus->Fill( lInvMassOmegaPlus );
            hLambdaMass->Fill(lInvMassLambdaAsCascDghter);
...
//-------------------------------------------------------
void AliAnalysisTaskQAMultistrangev2::Terminate(Option_t *) 
